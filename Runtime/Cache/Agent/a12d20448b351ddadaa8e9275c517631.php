...
<div id="content">
  ...
  <div id="content-header">
    <div id="breadcrumb"> <a href="<?php echo U('index/index');?>" title="返回首页" class="tip-bottom"><i class="icon-home"></i>首页</a><a href="#" class="current">路由器列表</a> </div>
    <h1>路由器列表</h1>
  </div>
  <!--End-breadcrumbs-->
            ...
            <table class="table table-bordered table-striped">
              ...
              <tbody>
               <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="<?php if(($mod) == "1"): ?>even gradeC<?php else: ?>odd gradeX<?php endif; ?>">
                  ...
                  <td>
                      <?php echo ($vo["gw_address"]); ?>/<?php echo ($vo["gw_port"]); ?>
                  </td>
                  <td>
                    <a href="<?php echo U('editroute',array('id'=>$vo['id'],'shopid'=>$vo['shopid']));?>" class="btn btn-primary btn-mini">编辑</a>
                                  |
                    <a href="javascript:drop_confirm('您确定要删除吗?','<?php echo U('delroute',array('id'=>$vo['id'],'shopid'=>$vo['shopid']));?>');"  class="btn btn-danger btn-mini">删除</a>
                  </td>
