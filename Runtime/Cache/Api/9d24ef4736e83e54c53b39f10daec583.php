...
<section id="wrapper" class="wrapper">
   ...
   <section class="slide-photos">
    <ul class="slides">
       <?php if(!empty($ad)): if(is_array($ad)): $i = 0; $__LIST__ = $ad;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div  class="swiper-slide">
	     		<?php if(($vo['mode']) == "1"): ?><li style="display: list-item; background-image: url(<?php echo ($vo["ad_thumb"]); ?>);">
	     				<a href="<?php echo U('userauth/showad',array('id'=>$vo['id']));?>">
   ...
   <input type="hidden" id="go_auth" value="<?php echo U('userauth/noAuth');?>"/>  
  ...
  <!--自定认证-->
		...
		<script language="javascript">	
			...
			function updateinfo(){					
				if(djwait == 0){
					var url=document.getElementById("go_auth").value;
